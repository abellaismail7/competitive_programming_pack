...
ll build(vector<ll> &t, int begin[], int end[], int index)
{
	...
	int diff = end - begin;
	t[index - 1] += build(t, begin, begin + diff/2, index * 2);
	t[index - 1] += build(t, begin + 1 + diff / 2, end, index * 2 + 1);
	return t[index - 1];
}

ll sum(vector<ll> &t, int pos, int tl, int tr, int l, int r)
{
	if (l > r)
		return 0;
	if (tl == l && tr == r)
		return t[pos - 1];
	int half = (tl+tr)/2;
	return sum(t, pos*2, tl, half, l, min(r, half)) + sum(t, pos*2+1, half+1, tr, max(l, half +1) , r);
}
...
int main (int argc, char *argv[])
{
	...
	while (q--)
	{
		...
		y--;
		std::cout << sum(t, 1, 0, n - 1, x, y) << std::endl;
	}
	//std::cout << "===" << std::endl;
	//for (auto e:t) {
	//	std::cout << e << std::endl;
